  ...
  <select onchange="this.form.submit()" class="form-select" name="champ_id" aria-label="Default select example">
    ...
    ?>
      <option value=<?php echo $ch->getId(); ?> <?php
                                                if ($ch->getId() == $currentChamp->getId()) {
                                                  echo " selected";
                                                }
                                                ?>><?php echo $ch->getName(); ?>
      </option>
...
<h2>Pontuação do campeonato <?php echo $currentChamp->getName(); ?></h2>
<table class="table">
  <thead>
    <tr>
    ...
    ?>
      <tr>
        <td><?php
            ...
            if ($team->points >= $currentChamp->getPtw()) {
            ?>
            <div class="alert alert-success" role="alert">
              Time Vencedor: <?php echo "{$team->name}";
                              $flag_win = 1; ?>
            </div>
          <?php
            } else {
...
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" <?php if ($flag_win == 1) {
                                                                                                      echo 'disabled';
                                                                                                    } ?>>
